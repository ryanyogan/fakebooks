...
export function CustomerCombobox({ error }: { error?: string | null }) {
  ...
  const comboBox = useCombobox<Customer>({
    id,
  ...
  const displayMenu = comboBox.isOpen && customers.length > 0;
          ...
          ? customers.map((customer, index) => (
              <li
                className={clsx("cursor-pointer py-1 px-2", {
                  "bg-green-200": comboBox.highlightedIndex === index,
                })}
                key={customer.id}
                {...comboBox.getItemProps({ item: customer, index })}
              >
